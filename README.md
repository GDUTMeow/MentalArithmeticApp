...
这是一个数学口算系统，能够在整数四则运算的范围内对学生的口算能力进行评价

## 特点

- 多账户，本系统可以给多个学生和多个老师同时使用
- 考试管理：分为考试新建、考试修改、考试删除，可以对考试的信息进行管理，新建考试的时候可以通过上传xlsx表格进行题目的导入
- 学生管理：分为学生新建、学生修改、学生删除，可以对学生的信息进行管理，新建学生的时候可以通过上传xlsx表格的方式进行学生的导入，还能够对学生进行重置密码的操作
- 成绩查看：不管是学生还是老师都可以进行成绩的查看，当然学生只能看到自己的，老师能够看到自己管理的学生的成绩
- 随机题目：老师可以选择考试的题目是否随机出现，能够有效防止串答案的情况出现
- 成绩导出：老师能够对自己负责的学生的成绩进行导出
...
- 修改了教师视图下添加学生页面的描述信息
- 修复了批量导入学生下对纯数字判断有错误的问题

### 2024/12/30

- 现在启动程序会自动打开浏览器了
- 添加学生添加过程中，名字长度的校验
- 对学生姓名添加了字符校验，限定为中文、英文、中文点 `·`、英文空格
- 对批量添加学生的返回逻辑进行了修改
- ~~在修改考试信息的时候，添加了更多的错误返回~~
- ~~在批量添加学生时，对学号是否为纯数字添加了校验~~
